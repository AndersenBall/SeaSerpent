...
public class BoatAlignNormal : FloatingObjectBase
{
    ...
    public void SetBoatSpeed(float enginePowr, float turnSpeed) {
        _enginePower = enginePowr;
        _turnPower = turnSpeed;
    }

    public float GetBoatEnginePower() {
        return _enginePower;
    }
    //sets speed and steering from outside source
    public void SetThrotal(float throtal) {
        _throttleBias = throtal ;
        
    }
    public float GetThrotal() {
        return _throttleBias;
    }
    public void SetSteer(float turn)
    {
        
        if (_throttleBias <= .5) {
            _steerBias = turn * _throttleBias * 2f;
        }
        if (_throttleBias > .5) {
            _steerBias = turn - turn * (_throttleBias - .5f)/3;
            //_steerBias = (turn * _throttleBias * 3/5) + (turn * 2/5);
        }
